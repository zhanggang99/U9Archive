...
{
    /// <summary>
    /// sql server 数据库访问助手类，本类为静态类，不可以被实例化，需要使用时直接调用即可 Copyright @zhanggangb@yonyou.com
    /// </summary>
    public static class SqlHelper
    {
        ...
        {
            string sql = "select count(1) from sysobjects where id = object_id('" + table + "') and OBJECTPROPERTY(id, 'IsUserTable') = 1";
        ...
        {
            string sql = "select count(1) from syscolumns where [id] = object_id('" + table + "') and[name] = '" + column + "'";
            object res = ExecuteScalar(sql);
            if (res == null)
                return false;
            return Convert.ToInt32(res) > 0;
        ...
        public static string GenerateQuerySql(string table, string[] columns, int index, int size, string where, string orderField, bool isDesc = true)
        {
            ...
            // SQL模版
            const string format = @"select {0} frm 
                                   (
        ...
        public static string GenerateQuerySql(string table, string[] columns, int size, string where, string orderField, bool isDesc = true)
        {
            ...
            return string.Format(format, size, column, table, where, string.IsNullOrEmpty(orderField) ? string.Empty : "order by " + orderField, isDesc && !string.IsNullOrEmpty(orderField) ? "desc" : string.Empty);
        }
        ...
        public static string GenerateQuerySql(string table, string[] columns, string where, string orderField, bool isDesc = true)
        {
            ...
            return string.Format(format, column, table, where, string.IsNullOrEmpty(orderField) ? string.Empty : " order by " + orderField, isDesc && !string.IsNullOrEmpty(orderField) ? "desc" : string.Empty);
        }
        #endregion
        #endregion
        ...
        {
            ExecuteReader(cmdText, action, parameters);
        } 
        #endregion
        ...
        /// <summary>
        /// 执行一个查询的T-SQL语句, 返回一个SqlDataReader对象, 如果出现SQL语句执行错误, 将会关闭连接通道抛出异常..( 注意：调用该方法后，一定要对SqlDataReader进行Close )
        /// </summary>
        /// <param name="cmdText"></param>
        /// <param name="parameters"></param>
        /// <returns></returns>
        ...
        /// <returns></returns>
        public static SqlDataReader ExecuteReader(string cmdText, CommandType type, params SqlParameter[] paramters)
        {
